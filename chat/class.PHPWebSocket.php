...
class PHPWebSocket
{
	...
	*/
	...
	function wsAddClient($socket, $clientIP, $nomePessoa) {
		...
		$this->wsClients[$clientID] = array($socket, '', self::WS_READY_STATE_CONNECTING, time(), false, 0, $clientIP, false, 0, '', 0, 0, $nomePessoa);
	...
	function log($mensagem){
		print '['.date('d.m.Y H:i').'] <b>Servidor</b> <i>'.$mensagem.'</i>\n';
	}

	function bind($tipo, $func){
		if(!isset($this->wsOnEvents[$tipo])){
			$this->wsOnEvents[$tipo] = array();
		}
		
		$this->wsOnEvents[$tipo][] = $func;
	}

	function unbind($tipo = ''){
		if($tipo){
			unset($this->wsOnEvents[$tipo]);
		}
		else{
